...
def _localize_unit( value, unit="temp" ):
    ...
    value = value.replace( chr(176), "" ).replace( "&deg;", "" ).replace( "mph", "" ).replace( "miles", "" ).replace( "mile", "" ).replace( "in.", "" ).replace( "F", "" ).replace( "AM", "" ).replace( "PM", "" ).replace( "am", "" ).replace( "pm", "" ).replace( "mi", "" ).strip()
...
class CurrentConditionsParser:
    def __init__( self, htmlSource, translate=None ):
        ...
        if ( htmlSource ):
            self._get_conditions( htmlSource )

    def _get_conditions( self, htmlSource ):
        # regex patterns
        ##http://www.weather.com/multimedia/videoplayer.html?from=email&bcpid=824514779&bclid=877032960&bctid=924200972
        ##var omn_locid="USMD0018";var omn_locytpe="1";var omn_locname="Baltimore";var omn_cityName="Baltimore";var omn_locstate="MD";var omn_zipcode="21202";var omn_loccountry="US";var omn_dmaCode="512";
        ##var omn_locid="USAK0025";var omn_locytpe="1";var omn_locname="Barrow";var omn_cityName="Barrow";var omn_locstate="AK";var omn_zipcode="99723";var omn_loccountry="US";var omn_dmaCode="";
        ##var omn_locid="USMI0564";var omn_locytpe="1";var omn_locname="Monroe";var omn_cityName="Monroe";var omn_locstate="MI";var omn_zipcode="48162";var omn_loccountry="US";var omn_dmaCode="505";
        # http://v.imwx.com/multimedia/video/wxflash/barrow.flv
        pattern_locstate = "var omn_locstate=\"([^\"]+)\""
        pattern_video_location = ">Watch the ([A-Za-z ]+) Forecast<"
        pattern_alerts = "alertArray.push\(new alertObj\('([^']+)','([^']+)','([^']+)'"
        pattern_conditions = """<td>Pressure:</td>.*?
.+?<td  class=\"ccWeaRes\">(.+?)
.+?<img src=\"http://i.imwx.com/web/common/icons/([^\"]+)\".*?
.+?<td>Visibility:</td>.*?
.+?<td class=\"ccWeaRes\">(.+?)</td>.*?
.+?<td>Sunrise:</td>.*?
.+?<td class=\"ccWeaRes\">(.+?)<BR>.*?
.+?<td>Sunset:</td>.*?
.+?<td class=\"ccWeaRes\">(.+?)<BR>.*?
.+?<div class=\"tnNPre\">Precip:(.+?)</div>"""
        # fetch conditions
        conditions = re.findall( pattern_conditions, htmlSource, re.DOTALL )
        # fetch video
        try:
            # fetch state
        ...
        # localize our conditions
        if ( conditions ):
            self.conditions += [ u"%s %s" % ( _localize_unit( conditions[ 0 ][ 0 ].replace( "&nbsp;", " " ), "pressure" ), { "up_pressure.gif": u"\u2191", "down_pressure.gif": u"\u2193", "steady_pressure.gif": u"\u2192" }[ conditions[ 0 ][ 1 ] ], ) ]
            self.conditions += [ _localize_unit( conditions[ 0 ][ 2 ], "distance" ) ]
            self.conditions += [ _localize_unit( conditions[ 0 ][ 3 ], "time" ) ]
            self.conditions += [ _localize_unit( conditions[ 0 ][ 4 ], "time" ) ]
            self.conditions += [ conditions[ 0 ][ 5 ].strip().replace( "%", "" ) ]
...
class WeatherClient:
    ...
    def fetch_current_conditions( self, video ):
        # fetch source
        htmlSource = self._fetch_data( self.BASE_FORECAST_URL % ( "today", self.code, "", ), 15, headers={ "Accept-Encoding": "text" } )
        # parse source for forecast
        parser = CurrentConditionsParser( htmlSource, self.translate )
        # fetch any alerts
        ...
        # return forecast
        return alerts, alertsrss, alertsnotify, alertscolor, len( parser.alerts), parser.conditions, video
    ...
    def fetch_weekend_forecast( self ):
        # fetch source
        htmlSource = self._fetch_data( self.BASE_FORECAST_URL % ( "weekend", self.code, "?lwsa=Weather36HourBusinessTravelerCommand", ), 15 )
        # parse source for forecast
    ...
    def fetch_map_list( self, maptype=0, userfile=None, locationindex=None ):
        ...
        if ( maptype == 0 ):
            ...
            #http://www.weather.com/weather/map/USMI0564
    ...
    def fetch_map_urls( self, map, userfile=None, locationindex=None ):
        ...
        if ( map.endswith( ".html" ) ):
            ...
            url = self.BASE_MAP_URL % ( "map", self.code, "&mapdest=%s" % ( map, ), )
        # fetch source
    ...
    def _fetch_data( self, base_url, refreshtime=0, filename=None, animated=False, subfolder="forecasts", retry=True, headers=None ):
        try:
            ...
            if ( not os.path.isfile( base_path ) or refresh ):
                ...
                # add faked header
                request.add_header( "Accept", "image/gif, image/jpeg, image/pjpeg, image/pjpeg, application/x-shockwave-flash, application/x-ms-application, application/x-ms-xbap, application/vnd.ms-xpsdocument, application/xaml+xml, */*" )
                request.add_header( "Accept-Language", "en-us" )
                request.add_header( "User-Agent", "Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; .NET CLR 1.1.4322; .NET CLR 2.0.50727)" )
                #request.add_header( "Accept-Encoding", "text" )
                #request.add_header( "Host", "www.weather.com" )
                request.add_header( "Connection", "Keep-Alive" )
                # add passed headers
...
if ( __name__ == "__main__" ):
    code = "USMI0564"#"USMD0018"#"USME0146"#USME0013"#"USMI0564"#"USWA0441"#"USNY0996"#"USOR0111"#"USOR0098"#"USGA0028"#"USAR0170"#"NZXX0006"#"USMI0405"#"USMI0508"#"NZXX0006"#"CAXX0343"#
    client = WeatherClient( code )
    """
    MAP_TYPE = 1
    MAP_NO = 0
    print "MAP LIST - %s" % client.BASE_MAPS[MAP_TYPE][0]
    category_title, map_list = client.fetch_map_list( MAP_TYPE, r"F:\source\_XBMC-Linux_\plugins\weather\weather.com plus\resources\samples\sample-user-defined-maps.udm", "1" )
    ...
    maps = client.fetch_map_urls( map_list[ MAP_NO ][ 0 ], r"F:\source\_XBMC-Linux_\plugins\weather\weather.com plus\resources\samples\sample-user-defined-maps.udm", map_list[ MAP_NO ][ 2 ] )
    print
    print maps
    print "IMAGES"
    ...
    alerts, alertsrss, alertsnotify, alertscolor, alerts_count, conditions, video = client.fetch_current_conditions( "" )
    print "Video:", video
